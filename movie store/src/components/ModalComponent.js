...
function validateInput() {
  ...
  for (var i = 0; i < arguments.length; i += 1) {
    ...
    if (input.value === "" || input.value.length === 0) {
      ...
      if (input.value.match(num) === null) {
        errors[errorName] = "vote_average must be number";
      }
    } else if (validationType === "vote_count") {
      if (input.value.match(num) === null) {
        errors[errorName] = "votercount field must contain number";
      }
    } else if (validationType === "popularity") {
      if (input.value.match(num) === null) {
        errors[errorName] = "popularity field must contian number";
      }
